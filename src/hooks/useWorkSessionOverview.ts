...
export interface WorkSessionOverview {
  currentRun: WorkSessionRun | null;
  historicalRuns: WorkSessionRun[];
  runs: WorkSessionRun[];
}
...
function compareRuns(left: WorkSessionRun, right: WorkSessionRun) {
  return (
...
export function buildWorkSessionOverview(
  ...
  const runs = [...runIds].map<WorkSessionRun>((runId) => {
...
/**
 * Work Session-level SidePanel view backed by the Project-scoped SQLite
 * journal projection.
 * ChatStore is intentionally not consulted here.
 */
export function useWorkSessionOverview(
  projectId: string | null | undefined
): WorkSessionOverview {
  const runtime = useProjectEventRuntime();
  const snapshot =
    projectId && runtime.projectId === projectId ? runtime.snapshot : null;
  return useMemo(() => buildWorkSessionOverview(snapshot), [snapshot]);
}
