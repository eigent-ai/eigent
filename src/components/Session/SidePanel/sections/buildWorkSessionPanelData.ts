...
import type { WorkSessionRun } from '@/hooks/useWorkSessionOverview';
import type {
  ChatActivityNode,
  ChatPlanTaskStatus,
  ChatProjectionNode,
...
export interface WorkSessionPanelData {
...
function collectAgents(
  ...
  for (const call of calls) {
    ...
    // independent agents and must not create a phantom "Remote sub-agent" row.
    ...
    const toolDisplayName = formatToolDisplayName(call.toolkitName);
    if (agent && !agent.tools.includes(toolDisplayName)) {
      agent.tools.push(toolDisplayName);
    }
  }
...
function collectProgress(runs: WorkSessionRun[]): SessionProgressItem[] {
...
function collectContext(
  ...
  for (const call of calls) {
    ...
    if (connector) {
      ...
      put(
        'connector',
        formatToolDisplayName(call.toolkitName),
        undefined,
        call
      );
    }
...
function collectResources(runs: WorkSessionRun[]): SessionResourceItem[] {
...
function collectFiles(runs: WorkSessionRun[]): SessionFileItem[] {
...
function collectEnvironments(runs: WorkSessionRun[]): SessionEnvironmentItem[] {
  const environments = new Map<string, SessionEnvironmentItem>();
  const put = (label: string, run: WorkSessionRun, time: number) => {
...
export function buildWorkSessionPanelData(
  runs: WorkSessionRun[],
  skills: ContextSkill[],
  connectors: ContextConnector[] = []
