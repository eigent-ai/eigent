						...
						if (key.toLowerCase() === 'notion') {
                            ...
                            onInstall = async () => {
                                try {
                                    ...
                                    if (response.success) {
                                        if (response.warning) {
                                            console.warn("Gmail connection warning:", response.warning);
                                        }
                                        try {
                                            const existingConfigs = await proxyFetchGet("/api/configs");
                                            const existing = Array.isArray(existingConfigs)
                                        ...
                                        console.log("Gmail installed successfully");
                                        const gmailItem = {
                                            id: 0, // Use 0 for integration items
                                            key: key,
                                            name: key,
                                            description: "Gmail integration for managing emails, drafts, labels, and contacts",
                                            toolkit: "google_gmail_toolkit", // Add the toolkit name
                                            isLocal: true
                                        };
                                        addOption(gmailItem, true);
                                    } else if (response.status === "authorizing") {
                                        console.log("Gmail authorization in progress. Please complete in browser.");
                                        if (response.message) {
                                            console.log(response.message);
                                        }
                                    } else {
                                        console.error("Failed to install Gmail:", response.error || "Unknown error");
                                        throw new Error(response.error || "Failed to install Gmail");
                                    }
                                    return response;
                                } catch (error: any) {
                                    if (!error.message?.includes("authorization")) {
