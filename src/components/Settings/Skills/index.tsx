...
const SKILL_TABLE_ROW_CLASS = [
  ...
  '[&>td]:bg-transparent [&>td]:transition-colors [&>td]:duration-150 motion-reduce:[&>td]:transition-none',
...
export const SKILLS_NOTICE_MINIMIZED_STORAGE_KEY =
  'eigent.skills.library-notice-minimized.v1';

function readSkillsNoticeMinimized(): boolean {
  if (typeof window === 'undefined') return false;
  try {
    return (
      window.localStorage.getItem(SKILLS_NOTICE_MINIMIZED_STORAGE_KEY) === '1'
    );
  } catch {
    return false;
  }
...
function rememberSkillsNoticeMinimized(): void {
  try {
    window.localStorage.setItem(SKILLS_NOTICE_MINIMIZED_STORAGE_KEY, '1');
  } catch {
    // The current page still minimizes the notice when storage is unavailable.
  }
}
...
export default function Skills() {
  ...
  const [noticeMinimized, setNoticeMinimized] = useState(
    readSkillsNoticeMinimized
  );
  ...
  const showCompactNotice =
    noticeMinimized && (errors.length > 0 || dismissedNoticeCount > 0);
        ...
        <SettingsSectionPage className="gap-ds-16 py-ds-24">
            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setNoticeMinimized(true);
                  rememberSkillsNoticeMinimized();
                }}
          ...
          {loading && entries.length === 0 ? (
            <SettingsSectionLoading
              label={t('agents.library-loading')}
              rows={5}
            />
          ) : (
            <div className="-m-ds-4 [&>div]:p-ds-4">
              <Table
                ...
                className="min-w-[760px] border-separate border-spacing-x-0 border-spacing-y-ds-4 text-start text-ds-text-base"
              >
                <TableHeader>
                ...
                <TableBody>
                  {visible.map((entry) => (
                    <TableRow
                      key={entry.id}
                      data-state={
                        selected.has(entry.id) ? 'selected' : undefined
                      }
                      className={SKILL_TABLE_ROW_CLASS}
                      onClick={() =>
                        setParams(skillSearch(entry.id), {
                      ...
                      <TableCell className="w-full max-w-0">
                        <div className="flex min-w-0 flex-col justify-center">
                          <Button
                            ...
                            size="sm"
                            textWeight="medium"
                            className="w-full min-w-0 !text-ds-ink-default-default no-underline hover:!text-ds-ink-default-default hover:no-underline"
                          >
                            <Link
          ...
          {entries.some((entry) => entry.kind === 'space') && (
            <DsText
              as="p"
              role="meta"
              className="m-0 text-ds-ink-muted-default"
            >
