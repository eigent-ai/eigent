...
const SPACE_MENU_SHELL_CLASS =
  'bg-ds-bg-neutral-subtle-default shadow-workspace-space-menu box-border inline-flex h-8 min-h-8 w-fit min-w-[180px] max-w-[300px] items-center gap-2 rounded-full px-3 py-0 font-semibold';

export interface WorkspaceSpaceMenuProps {
  /** Display-only: render the current Space name without the dropdown. */
  readOnly?: boolean;
}

/**
 * Space switcher for the workspace landing. Work Session switching lives in
 * the left sidebar; this control only creates/switches Spaces and exposes
 * actions backed by the active Project workdir contract.
 */
export function WorkspaceSpaceMenu({
  readOnly = false,
}: WorkspaceSpaceMenuProps) {
  ...
  const loadPendingOverlays = useCallback(async () => {
    ...
    try {
      ...
      console.warn(
        '[WorkspaceSpaceMenu] Failed to load pending changes:',
        error
      );
  ...
  const activateSpace = useCallback(
    async (spaceId: string) => {
      ...
      try {
        ...
        console.warn('[WorkspaceSpaceMenu] Failed to switch Space:', error);
  ...
  const handleRenameSpace = async () => {
    ...
    try {
      ...
      console.warn('[WorkspaceSpaceMenu] Failed to rename Space:', error);
    ...
    try {
      ...
      for (const [index, runId] of pendingRunIds.entries()) {
        ...
        try {
          ...
          console.warn(
            `[WorkspaceSpaceMenu] Failed to apply pending run ${runId}:`,
            error
          );
  ...
  const executeDiscardPending = async () => {
    ...
    try {
      ...
      console.warn(
        '[WorkspaceSpaceMenu] Failed to discard pending changes:',
        error
      );
  ...
  const handleRefreshWorkdir = async () => {
    ...
    try {
      ...
      console.warn(
        '[WorkspaceSpaceMenu] Failed to refresh project workdir:',
        error
      );
  ...
  const handleCreateSpaceFromFolder = async () => {
    try {
      ...
      console.warn(
        '[WorkspaceSpaceMenu] Failed to create folder Space:',
        error
      );
  ...
  if (readOnly) {
    return (
      <div
        className={cn(SPACE_MENU_SHELL_CLASS, 'justify-center')}
  ...
  return (
    <>
      ...
      <SpaceSwitchDropdown
        ...
        trigger={
          <Button
            id="workspace-space-menu-trigger"
            type="button"
            variant="ghost"
            size="md"
            buttonContent="text"
