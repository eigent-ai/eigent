...
import { AutomationTaskInput } from './AutomationTaskInput';
import DynamicAutomationConfig, {
  filterExcludedFields,
  getDefaultTriggerConfig,
  type TriggerConfigSchema,
  type ValidationError,
} from './DynamicAutomationConfig';
import { SchedulePicker, type ScheduleConfig } from './SchedulePicker';

type AutomationDialogProps = {
  selectedAutomation: Trigger | null;
...
export const AutomationDialog: React.FC<AutomationDialogProps> = ({
  ...
  const [formData, setFormData] = useState<TriggerInput>({
    ...
    trigger_type: selectedAutomation?.trigger_type || TriggerType.Schedule,
    custom_cron_expression:
      selectedAutomation?.custom_cron_expression || '0 0 * * *',
    listener_type: selectedAutomation?.listener_type || ListenerType.Workforce,
    webhook_method: selectedAutomation?.webhook_method || RequestType.POST,
    agent_model: selectedAutomation?.agent_model || '',
    task_prompt: selectedAutomation?.task_prompt || initialTaskPrompt || '',
    max_executions_per_hour: selectedAutomation?.max_executions_per_hour,
    max_executions_per_day: selectedAutomation?.max_executions_per_day,
  ...
  // Fetch the backend Trigger config only when a valid app is selected.
  const shouldFetchConfig =
    isOpen && (activeTab === 'schedule' || selectedApp !== '');
  const { data: configData } = useTriggerConfigQuery(
    selectedAutomation?.trigger_type || formData.trigger_type,
    shouldFetchConfig
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // If editing an existing Automation, populate the form with its data.
      if (selectedAutomation) {
        setFormData({
          ...
          custom_cron_expression:
            selectedAutomation.custom_cron_expression || '0 0 * * *',
          listener_type:
            selectedAutomation.listener_type || ListenerType.Workforce,
          webhook_method: selectedAutomation.webhook_method || RequestType.POST,
          agent_model: selectedAutomation.agent_model || '',
          task_prompt: selectedAutomation.task_prompt || '',
          max_executions_per_hour: selectedAutomation.max_executions_per_hour,
          max_executions_per_day: selectedAutomation.max_executions_per_day,
          webhook_url: selectedAutomation.webhook_url,
        });
        ...
        if (selectedAutomation.config) {
          setTriggerConfig(selectedAutomation.config as Record<string, any>);
          // For schedule triggers, also set scheduleConfig
          if (selectedAutomation.trigger_type === TriggerType.Schedule) {
            setScheduleConfig(selectedAutomation.config as ScheduleConfig);
          }
        } else {
        ...
        if (selectedAutomation.trigger_type === TriggerType.Slack) {
          setSelectedApp('slack');
          setActiveTab('app');
        } else if (selectedAutomation.trigger_type === TriggerType.Webhook) {
  ...
  }, [initialTaskPrompt, isOpen, selectedAutomation]);
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedAutomation) {
        ...
        response = await proxyUpdateTrigger(selectedAutomation.id, updateData);

        // Update trigger in store
        updateTrigger(selectedAutomation.id, response);
  ...
  const renderCreateContent = () => {
    const needsAuth =
      selectedAutomation?.status === TriggerStatus.PendingAuth &&
      selectedAutomation?.config?.authentication_required;
          ...
          >
            ...
            <TabsContent value="schedule" className="px-6 py-4">
              <SchedulePicker
                ...
                initialConfig={
                  selectedAutomation?.config as ScheduleConfig | undefined
                }
              />
            </TabsContent>
            <TabsContent value="app" className="px-6 py-4">
              {!selectedApp ? (
                ...
                <div className="space-y-4">
                  ...
                  {!selectedAutomation || !formData.webhook_url ? (
...
// Automation button component
type AutomationDialogButtonProps = {
  selectedAutomation?: Trigger | null;
  onTriggerCreating?: (triggerData: TriggerInput) => void;
...
export const AutomationDialogButton: React.FC<AutomationDialogButtonProps> = ({
