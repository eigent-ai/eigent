// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========

import { NavTab, SidebarScrollArea } from '@/components/Layout/AppSidebar';
import { ShortcutTooltipContent } from '@/components/ui/shortcut-tooltip';
import { cn } from '@/lib/utils';
import { Plus } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { SidebarAccordionSection } from './SidebarAccordionSection';
import {
  WorkSessionNavRows,
  type WorkSessionNavItem,
} from './WorkSessionNavRows';

export {
  WORK_SESSION_NAV_RECENT_MAX,
  WorkSessionNavRows,
  type WorkSessionNavItem,
} from './WorkSessionNavRows';

export interface WorkSessionNavListProps {
  workSessions: WorkSessionNavItem[];
  activeProjectId?: string | null;
  onWorkSessionClick?: (projectId: string) => void;
  onDeleteWorkSession?: (projectId: string) => void;
  onEndWorkSession?: (projectId: string) => void;
  onPinWorkSession?: (projectId: string) => void;
  onNewWorkSession: () => void;
  /** Selected state for the New Session row. */
  newWorkSessionActive?: boolean;
  className?: string;
}

/** New Session row, optional Pinned section, and Sessions section. */
export function WorkSessionNavList({
  workSessions,
  activeProjectId,
  onWorkSessionClick,
  onDeleteWorkSession,
  onEndWorkSession,
  onPinWorkSession,
  onNewWorkSession,
  newWorkSessionActive = false,
  className,
}: WorkSessionNavListProps) {
  const { t } = useTranslation();

  const newWorkSessionLabel = t('layout.new');
  const pinnedLabel = t('layout.pinned', { defaultValue: 'Pinned' });
  const workSessionsLabel = t('layout.projects', {
    defaultValue: 'Sessions',
  });

  const pinnedWorkSessions = workSessions.filter((item) => item.pinned);
  const unpinnedWorkSessions = workSessions.filter((item) => !item.pinned);
  const hasPinned = pinnedWorkSessions.length > 0;
  const hasUnpinned = unpinnedWorkSessions.length > 0;

  const sharedRowProps = {
    activeProjectId,
    onWorkSessionClick,
    onDeleteWorkSession,
    onEndWorkSession,
    onPinWorkSession,
    folded: false as const,
  };

  return (
    <div
      className={cn(
        'flex min-h-0 w-full min-w-0 flex-col overflow-hidden',
        className
      )}
    >
      {/* + New Session */}
      <div className="flex w-full min-w-0 flex-col">
        <NavTab
          active={newWorkSessionActive}
          onClick={onNewWorkSession}
          leading={<Plus className="h-4 w-4 shrink-0" aria-hidden />}
          label={newWorkSessionLabel}
          tooltip={
            <ShortcutTooltipContent
              label={newWorkSessionLabel}
              shortcutId="new-project"
            />
          }
          tooltipCompact
          tooltipVariant="delayed"
          ariaLabel={newWorkSessionLabel}
          ariaCurrentPage={newWorkSessionActive}
        />
      </div>

      {/* Scrollable section list */}
      <SidebarScrollArea className="m-0 mt-1 p-0 pb-1">
        {hasPinned && (
          <SidebarAccordionSection label={pinnedLabel}>
            <WorkSessionNavRows
              {...sharedRowProps}
              workSessions={pinnedWorkSessions}
            />
          </SidebarAccordionSection>
        )}
        {hasUnpinned && (
          <SidebarAccordionSection label={workSessionsLabel}>
            <WorkSessionNavRows
              {...sharedRowProps}
              workSessions={unpinnedWorkSessions}
            />
          </SidebarAccordionSection>
        )}
      </SidebarScrollArea>
    </div>
  );
}
