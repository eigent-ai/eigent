...
import { resolveWorkSessionNavLeadPresentation } from '@/lib/runNavLead';
...
import {
  AutomationReconnectButton,
  automationListenerLeadIconClass,
} from './AutomationNavTab';
import { WorkSessionNavList } from './WorkSessionNavList';
...
export default function SpaceSidebar({
  ...
  const [deleteWorkSessionProjectId, setDeleteWorkSessionProjectId] = useState<
    string | null
  >(null);
  const [deleteWorkSessionLoading, setDeleteWorkSessionLoading] =
    useState(false);
  const [endWorkSessionProjectId, setEndWorkSessionProjectId] = useState<
    string | null
  >(null);
  const [endWorkSessionLoading, setEndWorkSessionLoading] = useState(false);
  const [endWorkSessionDialogOpen, setEndWorkSessionDialogOpen] =
    useState(false);
  ...
  const shouldShowWorkSessionInNav = useCallback(
    (project: (typeof projectMetasForActiveSpace)[number]) => {
      ...
      if (!isPlaceholderProjectName(project.name, project.id)) {
        return true;
      }
  ...
  const isWorkSessionNavSelectionActive =
  ...
  const selectWorkSession = useCallback(
    async (projectId: string) => {
      ...
      // Already loaded — flip to the live Work Session shell immediately.
      if (
      ...
      // Work Session. It must not change the shared shell for the newer
      // selection.
  ...
  // Boot-time session resume: reopen the last visited Work Session (the way an
  ...
  // path as clicking the Work Session in the sidebar.
  useEffect(() => {
    ...
    if (!lastVisitedMeta || !shouldShowWorkSessionInNav(lastVisitedMeta)) {
      return;
    }
    void selectWorkSession(lastVisitedId);
  ...
  const workSessions = useMemo(
    () =>
      projectMetasForActiveSpace
        .filter((project) => shouldShowWorkSessionInNav(project))
        .map((project) => {
          ...
          return {
            ...
            title: resolveWorkSessionDisplayName(
              project.name,
              project.id,
              t('layout.new-project')
            ),
            runLead: resolveWorkSessionNavLeadPresentation({
    ...
    [
      ...
      projectMetasForActiveSpace,
      projectStore,
      shouldShowWorkSessionInNav,
      t,
    ]
  );

  const handleNewWorkSession = useCallback(() => {
  ...
  const handlePinWorkSession = useCallback((projectId: string) => {
  ...
  const requestDeleteWorkSession = useCallback((projectId: string) => {
    setDeleteWorkSessionProjectId(projectId);
  }, []);

  const requestEndWorkSession = useCallback((projectId: string) => {
    setEndWorkSessionProjectId(projectId);
    setEndWorkSessionDialogOpen(true);
  }, []);

  const confirmDeleteWorkSession = useCallback(async () => {
    const projectId = deleteWorkSessionProjectId;
    if (!projectId) return;

    setDeleteWorkSessionLoading(true);
    try {
      ...
      setDeleteWorkSessionLoading(false);
      setDeleteWorkSessionProjectId(null);
    }
  }, [
    activeSpaceId,
    deleteWorkSessionProjectId,
    email,
    ipcRenderer,
    projectStore,
    requestWorkspaceChatFocus,
  ...
  const confirmEndWorkSession = useCallback(async () => {
    const projectId = endWorkSessionProjectId;
    if (!projectId) return;

    setEndWorkSessionLoading(true);
    try {
      const wasActive = projectStore.activeProjectId === projectId;
      await ensureProjectLoaded(projectId);
      const projectChatStore = projectStore.peekActiveChatStore(projectId);
      ...
      console.error('[SpaceSidebar] Failed to end project:', error);
      toast.error(t('layout.failed-to-end-project'), {
        closeButton: true,
      });
    } finally {
      setEndWorkSessionLoading(false);
      setEndWorkSessionProjectId(null);
      setEndWorkSessionDialogOpen(false);
    }
  }, [
    endWorkSessionProjectId,
    ensureProjectLoaded,
    projectStore,
    requestWorkspaceChatFocus,
    setActiveWorkspaceTab,
  ...
  return (
    <>
      ...
      />
      <AlertDialog
        isOpen={deleteWorkSessionProjectId != null}
        onClose={() => {
          if (deleteWorkSessionLoading) return;
          setDeleteWorkSessionProjectId(null);
        }}
        onConfirm={() => void confirmDeleteWorkSession()}
        title={t('layout.delete-project')}
        message={t('layout.delete-project-confirmation')}
        confirmText={t('layout.delete')}
        cancelText={t('layout.cancel')}
        confirmDisabled={deleteWorkSessionLoading}
      />

      <AlertDialog
        isOpen={endWorkSessionDialogOpen}
        onClose={() => {
          if (endWorkSessionLoading) return;
          setEndWorkSessionDialogOpen(false);
          setEndWorkSessionProjectId(null);
        }}
        onConfirm={() => void confirmEndWorkSession()}
        ...
        confirmDisabled={endWorkSessionLoading}
      />
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
          <SidebarSection>
            <SidebarNavGroup>
                ...
                leading={(() => {
                  ...
                  return (
                    <ListenerIcon
                      className={cn(
                        'h-4 w-4 shrink-0',
                        automationListenerLeadIconClass(wsConnectionStatus)
                      )}
                      aria-hidden
                    />
                  );
              ...
              <NavTab
                ...
                ariaCurrentPage={isSpaceSettingsActive}
              />
            </SidebarNavGroup>
          </SidebarSection>
          ...
          <SidebarSection grow="fill">
            <WorkSessionNavList
              ...
              activeProjectId={
                isWorkSessionNavSelectionActive ? activeProjectId : null
              }
              onWorkSessionClick={selectWorkSession}
              onDeleteWorkSession={requestDeleteWorkSession}
              onEndWorkSession={requestEndWorkSession}
              onPinWorkSession={handlePinWorkSession}
              onNewWorkSession={handleNewWorkSession}
              newWorkSessionActive={activeWorkspaceTab === 'new-project'}
            />
          </SidebarSection>
        </div>
      </SidebarShell>
