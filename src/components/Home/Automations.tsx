// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========

import { AUTOMATION_ICON } from '@/lib/triggerIcon';
import {
  proxyActivateTrigger,
  proxyDeactivateTrigger,
  proxyDeleteTrigger,
} from '@/service/triggerApi';
import { Trigger, TriggerStatus, TriggerType } from '@/types';
import { useCallback, useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import HomeHubBoard from './components/HomeHubBoard';
import HomeHubBoardCard from './components/HomeHubBoardCard';
import HomeHubCard from './components/HomeHubCard';
import HomeHubGrid from './components/HomeHubGrid';
import HomeHubListItem from './components/HomeHubListItem';
import HomeHubListTable from './components/HomeHubListTable';
import { useHomeHub } from './context';
import { useHomeHubNavigation } from './hooks/useHomeHubNavigation';
import { useSpaceLabel } from './hooks/useSpaceLabel';
import { SpaceDetailListSkeleton } from './SpaceDetailLoadingSkeleton';
import {
  compareHubByName,
  compareHubByTimestamp,
  matchesHubNameSearch,
} from './utils';
import {
  getAutomationBoardColumn,
  groupByBoardColumn,
} from './utils/boardStatus';

function getAutomationTypeLabel(
  automation: Trigger,
  t: (key: string, options?: Record<string, unknown>) => string
) {
  switch (automation.trigger_type) {
    case TriggerType.Schedule:
      return t('triggers.schedule-trigger');
    case TriggerType.Webhook:
      return t('triggers.webhook-trigger');
    case TriggerType.Slack:
      return t('triggers.slack-trigger');
    default:
      return t('triggers.app-trigger');
  }
}

function AutomationRow({
  automation,
  viewMode,
  onEdit,
  onDelete,
  onToggleActive,
}: {
  automation: Trigger;
  viewMode: 'grid' | 'list' | 'board';
  onEdit: (automation: Trigger) => void;
  onDelete: (automation: Trigger) => void;
  onToggleActive: (automation: Trigger) => void;
}) {
  const { t } = useTranslation();
  const spaceLabel = useSpaceLabel(automation.space_id);
  const sharedProps = {
    kind: 'automation' as const,
    automation,
    spaceLabel,
    automationTypeLabel: getAutomationTypeLabel(automation, t),
    onEdit,
    onDelete,
    onToggleActive,
  };

  return viewMode === 'list' ? (
    <HomeHubListItem {...sharedProps} />
  ) : viewMode === 'board' ? (
    <HomeHubBoardCard {...sharedProps} />
  ) : (
    <HomeHubCard {...sharedProps} />
  );
}

interface AutomationsProps {
  automationsOverride?: Trigger[];
  presentation?: 'home' | 'space-detail';
}

export default function Automations({
  automationsOverride,
  presentation = 'home',
}: AutomationsProps = {}) {
  const { t } = useTranslation();
  const {
    viewMode,
    searchQuery,
    sortBy,
    sortDirection,
    automations: homeAutomations,
    automationsLoading,
    reloadAutomations,
  } = useHomeHub();
  const automations = automationsOverride ?? homeAutomations;
  const effectiveViewMode = presentation === 'space-detail' ? 'list' : viewMode;
  const effectiveSearchQuery =
    presentation === 'space-detail' ? '' : searchQuery;
  const { openAutomation } = useHomeHubNavigation();

  const filteredAutomations = useMemo(() => {
    const filtered = !effectiveSearchQuery.trim()
      ? automations
      : automations.filter((automation) =>
          matchesHubNameSearch(effectiveSearchQuery, automation.name)
        );

    return [...filtered].sort((a, b) => {
      if (sortBy === 'name') {
        return compareHubByName(a.name, b.name, sortDirection);
      }
      if (sortBy === 'updated') {
        return compareHubByTimestamp(
          a.updated_at || a.last_executed_at || a.created_at,
          b.updated_at || b.last_executed_at || b.created_at,
          sortDirection
        );
      }
      return compareHubByTimestamp(a.created_at, b.created_at, sortDirection);
    });
  }, [automations, effectiveSearchQuery, sortBy, sortDirection]);

  const handleEditAutomation = useCallback(
    (automation: Trigger) => {
      void openAutomation(automation);
    },
    [openAutomation]
  );

  const handleToggleActive = useCallback(
    async (automation: Trigger) => {
      try {
        if (automation.status === TriggerStatus.Active) {
          await proxyDeactivateTrigger(automation.id);
        } else {
          await proxyActivateTrigger(automation.id);
        }
        void reloadAutomations();
      } catch (error) {
        console.error('Failed to toggle trigger:', error);
      }
    },
    [reloadAutomations]
  );

  const handleDelete = useCallback(
    async (automation: Trigger) => {
      try {
        await proxyDeleteTrigger(automation.id);
        void reloadAutomations();
      } catch (error) {
        console.error('Failed to delete trigger:', error);
      }
    },
    [reloadAutomations]
  );

  const renderAutomationRow = useCallback(
    (automation: Trigger, mode: 'grid' | 'list' | 'board') => (
      <AutomationRow
        key={automation.id}
        automation={automation}
        viewMode={mode}
        onEdit={handleEditAutomation}
        onDelete={handleDelete}
        onToggleActive={handleToggleActive}
      />
    ),
    [handleDelete, handleEditAutomation, handleToggleActive]
  );

  const boardColumns = useMemo(() => {
    const grouped = groupByBoardColumn(
      filteredAutomations,
      getAutomationBoardColumn
    );

    return {
      default: grouped.default.map((automation) =>
        renderAutomationRow(automation, 'board')
      ),
      running: grouped.running.map((automation) =>
        renderAutomationRow(automation, 'board')
      ),
      awaiting_review: grouped.awaiting_review.map((automation) =>
        renderAutomationRow(automation, 'board')
      ),
    };
  }, [filteredAutomations, renderAutomationRow]);

  if (automationsLoading) {
    if (presentation === 'space-detail') {
      return <SpaceDetailListSkeleton kind="automation" />;
    }
    return (
      <div className="flex w-full min-w-0 flex-col">
        <div className="pb-12 text-body-sm text-ds-text-neutral-muted-default">
          {t('layout.loading')}
        </div>
      </div>
    );
  }

  return (
    <div className="flex w-full min-w-0 flex-col">
      <div className="mb-12 w-full min-w-0">
        {automations.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 text-center">
            <AUTOMATION_ICON className="mb-4 h-12 w-12 text-ds-icon-neutral-muted-default" />
            <div className="text-sm text-ds-text-neutral-muted-default">
              {t('triggers.no-triggers') || t('layout.triggers')}
            </div>
          </div>
        ) : filteredAutomations.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 text-center">
            <div className="text-sm text-ds-text-neutral-muted-default">
              {t('layout.search-no-results')}
            </div>
          </div>
        ) : effectiveViewMode === 'board' ? (
          <HomeHubBoard columns={boardColumns} />
        ) : effectiveViewMode === 'grid' ? (
          <HomeHubGrid>
            {filteredAutomations.map((automation) =>
              renderAutomationRow(automation, 'grid')
            )}
          </HomeHubGrid>
        ) : (
          <HomeHubListTable kind="automation">
            {filteredAutomations.map((automation) =>
              renderAutomationRow(automation, 'list')
            )}
          </HomeHubListTable>
        )}
      </div>
    </div>
  );
}
