// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========

import VerticalNavigation, {
  type VerticalNavItem,
} from '@/components/Dashboard/VerticalNav';
import {
  buildHistorySearchString,
  type BrowserSection as ShellBrowserSection,
} from '@/lib/historyNavConfig';
import { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useNavigate, useSearchParams } from 'react-router-dom';
import DashboardPageLayout from '../DashboardPageLayout';
import CDP from './CDP';
import Cookies from './Cookies';
import Extension from './Extension';

const BROWSER_SECTIONS = ['cdp', 'extension', 'cookies'] as const;
type BrowserSection = (typeof BROWSER_SECTIONS)[number];

function isBrowserSection(value: string | null): value is BrowserSection {
  return value !== null && BROWSER_SECTIONS.includes(value as BrowserSection);
}

function shellToLocal(
  v: ShellBrowserSection | undefined
): BrowserSection | null {
  if (v && BROWSER_SECTIONS.includes(v as BrowserSection)) {
    return v as BrowserSection;
  }
  return null;
}

export default function Browser({
  embedded,
  browserSection: sectionControlled,
}: {
  embedded?: boolean;
  browserSection?: ShellBrowserSection;
}) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const sectionFromUrl = searchParams.get('browserSection');

  const [activeTabInternal, setActiveTabInternal] = useState<BrowserSection>(
    () => (isBrowserSection(sectionFromUrl) ? sectionFromUrl : 'cdp')
  );

  const resolvedTab: BrowserSection =
    embedded && sectionControlled != null
      ? (shellToLocal(sectionControlled) ?? 'cdp')
      : embedded
        ? isBrowserSection(sectionFromUrl)
          ? sectionFromUrl
          : 'cdp'
        : activeTabInternal;

  useEffect(() => {
    if (searchParams.get('browserAction') !== 'launch') return;
    setActiveTabInternal('cdp');
  }, [searchParams]);

  /** Deep-link cleanup: strip consumed `browserSection` unless History shell owns URL. */
  useEffect(() => {
    if (embedded) return;
    if (!isBrowserSection(sectionFromUrl)) return;
    setActiveTabInternal(sectionFromUrl);
    const next = new URLSearchParams(searchParams);
    next.delete('browserSection');
    setSearchParams(next, { replace: true });
  }, [embedded, sectionFromUrl, searchParams, setSearchParams]);

  useEffect(() => {
    if (embedded || !isBrowserSection(sectionFromUrl)) return;
    setActiveTabInternal(sectionFromUrl);
  }, [embedded, sectionFromUrl]);

  const menuItems = [
    {
      id: 'cdp',
      name: t('layout.browser-connections'),
    },
    {
      id: 'extension',
      name: t('layout.browser-plugins'),
    },
    {
      id: 'cookies',
      name: t('layout.browser-cookie'),
    },
  ];

  const handleTabChange = (tabId: string) => {
    if (!isBrowserSection(tabId)) return;
    const next = tabId as BrowserSection;
    if (!embedded) {
      setActiveTabInternal(next);
    }
    if (embedded) {
      navigate(
        {
          pathname: '/history',
          search: `?${buildHistorySearchString(searchParams, {
            tab: 'browser',
            browserSection: next,
          })}`,
        },
        { replace: true }
      );
    }
  };

  const pageTitle =
    resolvedTab === 'cdp'
      ? t('layout.browser-connections')
      : resolvedTab === 'extension'
        ? t('layout.browser-plugins')
        : t('layout.browser-cookie');

  const mainPanels = (
    <div className="gap-4 flex flex-col">
      {resolvedTab === 'cdp' && <CDP />}
      {resolvedTab === 'extension' && <Extension />}
      {resolvedTab === 'cookies' && <Cookies />}
    </div>
  );

  return (
    <div className="flex h-auto w-full">
      {!embedded && (
        <div className="top-20 w-40 pr-6 pt-8 sticky flex h-full flex-shrink-0 flex-grow-0 flex-col justify-between self-start">
          <VerticalNavigation
            items={
              menuItems.map((menu) => ({
                value: menu.id,
                label: (
                  <span className="text-body-sm font-bold">{menu.name}</span>
                ),
              })) as VerticalNavItem[]
            }
            value={resolvedTab}
            onValueChange={handleTabChange}
            className="min-h-0 gap-0 h-full w-full flex-1"
            listClassName="w-full h-full overflow-y-auto"
            contentClassName="hidden"
          />
        </div>
      )}

      {embedded ? (
        <DashboardPageLayout className="min-w-0 flex-1" title={pageTitle}>
          {mainPanels}
        </DashboardPageLayout>
      ) : (
        <div className="min-w-0 flex h-auto flex-1 flex-col">{mainPanels}</div>
      )}
    </div>
  );
}
