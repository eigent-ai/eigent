...
/**
 * Aggregate-only unfinished Runs can be stale after a crash or bounded
 * hydration. A retained canonical node proves this frontend observed part of
 * the current durable Run journal rather than only a legacy/past aggregate.
 */
...
/**
 * `requestSource: canonical` only describes the transport envelope. A legacy
 * `legacy.step` ASK can use that envelope too, so require the durable request
 * provenance to identify one of the typed request families.
 */
 ...
 *
 * A canonical pending request is an explicit user obligation and therefore
 * outranks the legacy ChatStore's active Run. Without one, only the currently
 * legacy-owned live local Run may inherit controls. Orphan projected Runs do
 * not get a recency fallback: historical, read-only, aggregate-only, and
 * resyncing Runs fail closed.
 */
...
/** A local interruption is actionable only while its journal coverage is visible. */
