...
import { getBottomBoxStateForTask } from '@/lib/taskLifecycleUi';
...
export default function ChatBox(): JSX.Element {
  ...
  // Determine BottomBox state (shared with ProjectPageSidebar via taskLifecycleUi)
  const getBottomBoxState = () => {
    if (!chatStore.activeTaskId) return 'input';
    const task = chatStore.tasks[chatStore.activeTaskId];
    return getBottomBoxStateForTask(task);
  };
  ...
  return (
    <div className="rounded-2xl border-border-tertiary bg-surface-secondary flex h-full w-full min-w-[360px] flex-none items-center justify-center overflow-hidden border-solid">
