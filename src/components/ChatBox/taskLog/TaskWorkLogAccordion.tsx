 ...
 *              endCursor[agentId] ?? 0  (closed group)
 */
...
export interface TaskWorkLogAccordionProps {
  ...
  startCursor?: WorkLogCursor;
  /** Show only log entries produced before this cursor (exclusive). Absent = active group. */
  endCursor?: WorkLogCursor;
...
export function TaskWorkLogAccordion({
  ...
  useEffect(() => {
    if (status === ChatTaskStatus.FINISHED || frozen) {
      setOuterOpen(false);
