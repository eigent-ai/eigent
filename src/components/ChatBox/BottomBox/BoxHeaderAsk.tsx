...
// ---------------------------------------------------------------------------
// Render a single HitlInputBlock
// ---------------------------------------------------------------------------
    ...
    case 'file_upload':
      return (
        <FileUploadBlock
          key={block.id}
          block={block}
          value={(value as globalThis.File[]) ?? []}
          onChange={(v) => onChange(v)}
          disabled={disabled}
        />
      );
...
export const BoxHeaderAsk: React.FC<BoxHeaderAskProps> = ({
  ...
  const handleSubmit = useCallback(
    (reply?: string) => {
      if (submitted) return;
      // A direct reply string (from a choice/text block pressing Enter or a button) — use as-is.
      if (reply !== undefined) {
        setSubmitted(true);
        onSubmit(reply);
        return;
      }
      // Submit button clicked: for a single text block, extract the value and validate.
      ...
      // Multi-block: package all values as JSON.
      setSubmitted(true);
  ...
  // Single-block choice or text: can submit directly from the block.
  // Multi-block: show an explicit Submit button.
  const showSubmitButton =
    askPayload.inputs.length > 1 ||
    (askPayload.inputs.length === 1 &&
      askPayload.inputs[0]!.kind !== 'choice' &&
      askPayload.inputs[0]!.kind !== 'redirect');
  ...
  return (
    <div className="flex flex-col">
      {/* ── Viewing section ──────────────────────────────────────────── */}
      <div className="border-ds-border-neutral-default-default px-4 pt-3 pb-2 border-b">
        <div className="gap-1.5 mb-1 flex items-center">
          <CircleUser
            size={13}
      ...
      {/* ── Action section ───────────────────────────────────────────── */}
      <div className="gap-3 px-4 pt-3 pb-2 flex flex-col">
        {askPayload.inputs.map((block) =>
          renderBlock({
            block,
            value: values[block.id],
            onChange: (v) => setBlockValue(block.id, v),
            // Pass onSubmit only for single-input choice/text (auto-submits on selection).
            onSubmit:
              askPayload.inputs.length === 1 &&
...
// Re-export the AskQuestion type for backward compat with any remaining
// consumers (BottomBox/index.tsx) that may reference AskQuestion.
export type { AskPayload as AskQuestion };
