...
const RUNTIME_READINESS_ISSUE_MESSAGES: Readonly<Record<string, string>> = {
  ...
  multi_agent_runtime_adapter_unavailable:
    'This Space profile requires multi-agent runtime support that is not available.',
  ...
  local_setup_incomplete:
    'Required local values, files, connections, or approvals are incomplete.',
  workspace_bundle_not_materialized:
    'Space files and settings have not finished importing.',
};
...
function RuntimeReadinessStatus({
    ...
    return (
      <div className="mt-4 rounded-xl bg-ds-bg-success-subtle-default p-3 text-body-sm text-ds-text-success-default-default">
        ...
        <p className="mt-1 text-body-xs">
          Brain preflight confirmed these imported Space settings can start a
          Run.
        </p>
      </div>
    );
  ...
  return (
    <div className="mt-4 rounded-xl bg-ds-bg-neutral-subtle-default p-3 text-body-sm">
      ...
      <p className="mt-1 text-body-xs text-ds-text-neutral-muted-default">
        Space files and local bindings are installed, but runtime readiness has
        not been verified.
      </p>
    </div>
  );
    ...
    <div className="rounded-xl border border-ds-border-warning-default-default p-3">
      ...
      <dl className="mt-3 grid gap-3 text-body-xs">
        ...
        <div>
          ...
          <dd className="mt-1 break-all font-mono">
            {destination.cwd_scope || 'Space default'}
          </dd>
        </div>
        <div>
      ...
      {!approved && !canApprove ? (
        <p className="mt-3 rounded-lg bg-ds-bg-error-subtle-default p-2 text-body-xs text-ds-text-error-strong-default">
          {destination.destination_kind === 'stdio_unstable'
            ? 'This MCP executable is not pinned to a reviewed Space profile file and cannot be approved or started.'
...
export interface SpaceProfileImportWizardProps {
...
export function SpaceProfileImportWizard({
  initialHandle = '',
  initialProposalId = '',
  targetSpaceId,
  showHeader = true,
  ...
  const reviewedSetup = useMemo(() => {
    ...
    return [
      ...
      ...spec.context
        ...
        .map((source) => `Local files: ${source.slot}`),
      ...spec.connectors.map(
        (connector) =>
    ...
    try {
      ...
      if (!seed) {
        ...
        if (!root) {
          ...
          throw new Error(
            'Eigent could not create local files for this Space.'
          );
        }
      }
      ...
      {showHeader ? (
        <header>
          <h1 className="text-heading-2xl font-semibold">
            Import Space profile
          </h1>
          <p className="mt-2 text-body-sm text-ds-text-neutral-muted-default">
            Review what the Space can access, then configure required values and
            connections locally.
          </p>
        </header>
      ) : null}
        ...
        <div className="rounded-xl border border-ds-border-error-default-default bg-ds-bg-error-subtle-default p-4 text-body-sm text-ds-text-error-strong-default">
          ...
          {retryMode === 'start' && installSeed ? (
            <p className="mt-2 text-ds-text-neutral-muted-default">
              The inactive Space draft was kept for recovery. Retry reuses the
              same draft and does not create another Space.
            </p>
          ) : null}
          {retryMode ? (
            <Button
          ...
          <CardContent className={showHeader ? undefined : '!p-0'}>
            ...
            >
              <Input
                ...
                aria-label="Space profile share handle"
              />
      ...
      {snapshot && proposal ? (
        <>
          {proposal.state === 'materialized' ? (
            <Card>
              <CardHeader>
                <CardTitle>Space files imported</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-body-sm text-ds-text-neutral-muted-default">
                ...
                />
                <Button className="mt-4" onClick={openSpace}>
                  Open Space
                </Button>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="rounded-xl border border-ds-border-neutral-subtle-default p-3 text-body-sm">
                  ...
                  {proposal.install_plan.local_path_slots.map((slotId) => (
                    <p key={slotId}>Local files access: {slotId}</p>
                  ))}
          ...
          {['approved', 'needs_attention', 'materialized'].includes(
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>2. Local files and connections</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {proposal.install_plan.local_path_slots.map((slotId) => (
                    <div
                  ...
                  proposal.install_plan.connector_slots.length === 0 ? (
                    <p className="text-body-sm text-ds-text-neutral-muted-default">
                      No files or connector access required.
                    </p>
                  ) : null}
                </CardContent>
              </Card>
                  ...
                  {proposal.state === 'materialized' ? (
                    ...
                    >
                      ...
                      {proposal.error_code === 'bundle_reconfiguration_pending'
                        ...
                        : 'Import Space files and settings'}
                    </Button>
                  )}
                </CardContent>
              </Card>
