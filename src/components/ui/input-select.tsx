...
const InputSelect = React.forwardRef<HTMLInputElement, InputSelectProps>(
  (
    ...
    const interactingWithDropdownRef = React.useRef(false);
    ...
    // Reset dropdown interaction flag on mouse release.
    React.useEffect(() => {
      if (!isOpen) return;
      const handleMouseUp = () => {
    ...
    return (
      <div className={cn('relative w-full', stateCls.wrapper, className)}>
        {title && (
        ...
        <div
          ...
          className={cn(
            'relative flex w-full cursor-text items-center gap-2 rounded-lg border border-solid px-3 text-text-body shadow-sm outline-none transition-all',
            sizeClasses[size],
            stateCls.container,
            !disabled &&
              state !== 'error' &&
        ...
        >
          {leadingIcon && (
            <span className="h-4 w-4 flex-shrink-0 text-icon-primary">
              {leadingIcon}
            </span>
          )}
          <input
          ...
          <ChevronDown
            className={cn(
              'h-4 w-4 flex-shrink-0 text-icon-primary transition-transform',
              isOpen && 'rotate-180'
            )}
          />
        </div>
        ...
        {isOpen && (
          <div
            ...
            className="absolute left-0 right-0 top-full z-50 mt-1 overflow-hidden rounded-lg border border-solid border-transparent bg-input-bg-default shadow-md"
          >
            <div
            ...
            >
              {options.map((option) => (
                <div
                  ...
                  className={cn(
                    'relative flex w-full cursor-pointer select-none items-center rounded-lg py-1.5 pl-2 pr-8 text-sm outline-none transition-colors hover:bg-menutabs-fill-hover',
