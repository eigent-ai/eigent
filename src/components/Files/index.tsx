...
function treeSegmentLabel(value: string | null | undefined, projectId: string) {
  return resolveWorkSessionDisplayName(
    value,
    projectId,
    `Session ${projectId}`
  ).replace(/[\\/]/g, '-');
    ...
    const setFileList = async (
      ...
      if (
        ...
        try {
          ...
          console.warn(
            '[FilesBrowser] Failed to fetch local project files:',
            error
          );
        }
      }
      ...
      if (
        ...
        try {
          ...
          if (!baseURL) {
            console.warn(
              '[FilesBrowser] Brain not connected, cannot fetch files'
            );
          } else {
          ...
          console.warn(
            '[FilesBrowser] Failed to fetch files from Brain:',
            error
          );
        }
      }
          ...
          {selectedFile ? (
            !loading ? (
              selectedFile.isFolder ? (
                ...
                <FileContentPreview selectedFile={selectedFile} />
              ) : ['html', 'htm'].includes(selectedType) ? (
                isShowSourceCode ||
                selectedFile.preview?.kind === 'truncated-text' ? (
                  <SourceFilePreview
