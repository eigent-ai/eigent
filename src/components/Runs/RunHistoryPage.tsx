...
import { RunNavRows, type RunNavItem } from './RunNavRows';
  ...
  const runs: RunNavItem[] = useMemo(() => {
    const entries = Object.entries(tasks)
      ...
      .map(([id, task]) => ({
        ...
        runLead: getRunNavLeadPresentation(task),
        source: task.source,
      }));
    entries.sort(
      (a, b) => taskIdToCreatedMs(b.runId) - taskIdToCreatedMs(a.runId)
    );
    return entries;
  }, [tasks, t]);
