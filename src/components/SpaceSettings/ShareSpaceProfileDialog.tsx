...
type SpaceProfileVisibility = 'private' | 'public';

const isPublishableVisibility = (
  value: string
): value is SpaceProfileVisibility => value === 'private' || value === 'public';

interface ShareSpaceProfileDialogProps {
  open: boolean;
...
const errorMessage = (error: unknown): string => {
  ...
  return 'The Space profile could not be published. Try again.';
};
...
export function ShareSpaceProfileDialog({
  open,
  onOpenChange,
  spaceId,
  identity,
  ...
  const [visibility, setVisibility] =
    useState<SpaceProfileVisibility>('private');
  const [knownCloudProfile, setKnownCloudProfile] =
  ...
  const loadReview = useCallback(async () => {
    ...
    try {
      ...
      if (response.draft_version !== draft.version) {
        throw new Error(
          'The local Space settings changed. Close this review and wait for them to save.'
        );
      }
      ...
      if (existing) {
        if (existing.workspace_id !== spaceId) {
          throw new Error(
            'This Space profile name belongs to a different Space.'
          );
        }
        if (!isPublishableVisibility(existing.visibility)) {
          throw new Error(
            'Team sharing is not available without team publishing authority. Choose a private or public Space profile instead.'
          );
        }
        setVisibility(existing.visibility);
        setKnownCloudProfile(existing);
  ...
  const assetLimitError = review
    ? totalAssetCount > MAX_ASSET_COUNT
      ? `A Space profile can contain at most ${MAX_ASSET_COUNT} files.`
      : totalAssetBytes > MAX_TOTAL_ASSET_BYTES
        ? 'Selected files exceed the 128 MiB total Space profile limit.'
        : Object.values(assetFiles).some(
              (file) => file.size > MAX_ASSET_BYTES
            ) ||
            preparedAssets.some((asset) => asset.size_bytes > MAX_ASSET_BYTES)
          ? 'A Space profile file exceeds the 16 MiB per-file limit.'
          : null
    : null;
    ...
    try {
      ...
      const selected = manualAssetPaths.map((path) => {
        ...
        if (file.size > MAX_ASSET_BYTES) {
          throw new Error(
            `${file.name} exceeds the 16 MiB Space profile file limit.`
          );
        }
        return { path, file };
      });
      ...
      if (
        ...
        throw new Error(
          'Cloud validation does not match the reviewed local Space settings.'
        );
      }
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader
          title="Publish Space profile"
          subtitle="Review the portable Space settings before publishing an immutable version."
        />
            ...
            <>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-body-xs text-ds-text-neutral-muted-default">
                      ...
                      contain public setting values; local secret slot values
                      are never included.
                    </p>
                  </div>
                </div>
              ...
              <section className="space-y-2">
                ...
                {!requirementsReady ? (
                  <div className="rounded-xl border border-ds-border-warning-default-default bg-ds-bg-warning-subtle-default p-3 text-body-sm">
                    <p>
                      Local Space settings revealed undeclared or insufficiently
                    ...
                    >
                      Add safe requirements to Space settings
                    </Button>
                  </div>
                ) : null}
              </section>
              ...
              <section className="space-y-2">
                ...
                {recoverablePublishedRevision ? (
                  ...
                  <p className="text-body-xs text-ds-text-neutral-muted-default">
                    Eigent does not scan or upload ordinary Space files
                ...
                {recoverablePublishedRevision ? null : totalAssetCount === 0 ? (
                  <p className="rounded-xl bg-ds-bg-neutral-subtle-default p-3 text-body-xs text-ds-text-neutral-muted-default">
                    This Space profile has no files.
                  </p>
                  ...
                  <div className="space-y-3 rounded-xl border border-ds-border-neutral-subtle-default p-3">
                    <div>
                      ...
                      <p className="mt-1 text-body-xs text-ds-text-neutral-muted-default">
                        ...
                        stay outside the renderer and ordinary Space files are
                        not included.
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <span className="mt-1 block text-body-xs text-ds-text-neutral-muted-default">
                        {option === 'private'
                          ...
                          : 'Anyone with access to the Space profile can review and import it.'}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              {publishedHandle ? (
                <div className="rounded-xl border border-ds-border-success-default-default bg-ds-bg-success-subtle-default p-4">
                  ...
                  <p
                    ...
                    aria-label="Published Space profile handle"
                  >
                    {publishedHandle}
                  </p>
                  <Button
                  ...
                  <p className="mt-2 text-body-xs text-ds-text-neutral-muted-default">
                    ...
                    coordinate. Recipients can paste it into Import Space
                    profile to review and import this immutable version.
                  </p>
                  <p className="mt-2 text-body-xs text-ds-text-neutral-muted-default">
