...
export type SpaceResourceEditorState =
  | {
      kind: 'environment';
      mode: EditorMode;
      step: 'editor';
...
interface SpaceResourceEditorPanelProps {
  editor: SpaceResourceEditorState;
  document: WorkspaceConfigurationDocument;
  saveState: 'idle' | 'loading' | 'saving' | 'saved' | 'needs_attention';
  onChange: (editor: SpaceResourceEditorState) => void;
  onClose: () => void;
  onCommit: () => void;
  onDelete: () => void;
}
...
const referenceSourceKindLabel = (kind: WorkspaceContextSource['kind']) =>
  ({
    bundle_asset: 'Profile file',
    inline: 'Inline text',
    connection_query: 'Connection query',
    local_path_slot: 'Local files',
    artifact_ref: 'Artifact reference',
    memory_scope: 'Memory scope',
  })[kind];

export const referenceSourceDraftForKind = (
  id: string,
  ...
  if (editor.kind === 'context') {
    const options: Array<{
    ...
    }> = [
      {
        kind: 'local_path_slot',
        title: 'Local files',
        description: 'Ask each recipient to connect local files.',
        icon: <FolderOpen className="h-4 w-4" aria-hidden />,
      },
      {
        kind: 'bundle_asset',
        title: 'Profile file',
        description: 'Reference a file included in this Space profile.',
        icon: <FileText className="h-4 w-4" aria-hidden />,
      },
      {
        kind: 'inline',
        title: 'Inline text',
        description: 'Store a short, portable reference note.',
        icon: <FileText className="h-4 w-4" aria-hidden />,
      },
      {
        kind: 'connection_query',
        title: 'Connection query',
        description: 'Retrieve a reference through a configured connection.',
          ...
          <PickerOption
            ...
            onClick={() =>
              onChange({
                ...editor,
                step: 'editor',
                item: referenceSourceDraftForKind(editor.item.id, option.kind),
                queryText: '{}',
    ...
    return (
      <div className="space-y-2" data-workspace-resource-picker="skill">
        ...
        <PickerOption
          icon={<FileText className="h-4 w-4" aria-hidden />}
          title="Profile skill"
          description="Reference a skill included in this Space profile."
          onClick={() =>
            onChange({
              ...editor,
              step: 'editor',
    ...
    return (
      <div className="space-y-2" data-workspace-resource-picker="mcp">
        ...
        <PickerOption
          ...
          description="Reference an MCP definition included in this Space profile."
          onClick={() =>
            onChange({
              ...editor,
              step: 'editor',
...
function EditorFields({
  ...
  if (editor.kind === 'context') {
    const changeKind = (kind: WorkspaceContextSource['kind']) =>
      onChange({
        ...editor,
        item: referenceSourceDraftForKind(editor.item.id, kind),
        queryText: '{}',
        queryError: undefined,
      });
    return (
        ...
        >
          ...
          <SelectContent>
            <SelectGroup>
              ...
              <SelectItem value="bundled">Included in profile</SelectItem>
...
export function SpaceResourceEditorPanel({
  editor,
  document,
  saveState,
  onChange,
  ...
  const handleEditorChange = useCallback(
    (nextEditor: SpaceResourceEditorState) => {
          ...
          <div className="min-w-0">
            ...
            <span className="mt-1 block text-body-xs text-ds-text-neutral-muted-default">
              {editor.step === 'picker'
                ...
                : editor.mode === 'create'
                  ? `Configure this ${resourceName} before adding it to the Space profile.`
