...
export const TriggerListItem: React.FC<TriggerListItemProps> = ({
  ...
  return (
    <div
      ...
      className={`group flex cursor-pointer items-center gap-3 rounded-xl border bg-surface-primary p-3 transition-all duration-200 ${
        isSelected
    ...
    >
      ...
      <div className="bg-amber-500/10 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg">
        <Zap className="h-5 w-5 text-icon-primary" />
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 truncate text-xs text-text-label">
          {trigger.task_prompt ||
            trigger.description ||
      ...
      {/* 3. Trigger Type */}
      <div className="flex min-w-[80px] items-center gap-1.5 text-xs text-text-label">
        {getTriggerTypeIcon()}
