...
import { classifyError } from '@/lib/events/appEventClassifiers';
import {
  recordMcpInstalled,
  recordMcpInstallFailed,
  ...
  const installMcp = async (id: number) => {
    ...
    try {
      ...
      recordMcpInstallFailed({
        mcp_id: id,
        mcp_key: mcpItem?.key,
        // Bucketed on-device: raw messages can embed local paths / command
        // output (user content) and are unbounded-cardinality.
        error_type: classifyError(e instanceof Error ? e.message : String(e)),
