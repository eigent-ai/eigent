...
export default function SettingMCP() {
  ...
  // Download skill file
  const handleSkillDownload = async (skill: SkillInfo) => {
    try {
      // Use local backend API
      const baseURL = await getBaseURL();
      const url = `${baseURL}/skills/${encodeURIComponent(skill.name)}`;
  ...
  const handleSkillDelete = async () => {
    ...
    try {
      // Use local backend API
      const baseURL = await getBaseURL();
      const url = `${baseURL}/skills/${encodeURIComponent(skillToDelete.name)}`;
      const response = await fetch(url, {
        method: 'DELETE',
      });
                  ...
                  {!collapsedSkills && (
                    <>
                      ...
                      {!skillsLoading && (
                        <SkillList
                          skills={skills}
                          onDownload={handleSkillDownload}
