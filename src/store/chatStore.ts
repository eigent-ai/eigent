...
const resolveProcessTaskIdForToolkitEvent = (tasksById: Record<string, Task>, currentTaskId: string, agentName: string | undefined, processTaskId: unknown) => {
	...
	const match = running.findLast((t: any) => typeof t?.id === "string" && t.id && (agentName ? t.agent?.type === agentName : true));
...
const chatStore = (initial?: Partial<ChatStore>) =>
	createStore<ChatStore>()((set, get) => ({
		...
		computedProgressValue(taskId: string) {
			...
			const finishedTask = taskRunning?.filter((task) => task.status === "completed" || task.status === "failed").length;
			const taskProgress = (((finishedTask || 0) / (taskRunning?.length || 0)) * 100).toFixed(2);
		...
		startTask: async (taskId: string, type?: string, shareToken?: string, delayTime?: number, messageContent?: string, messageAttaches?: File[]) => {
			...
			if (modelType === "custom") {
				try {
					...
					const googleApiKey = configs.find((c: any) => c.config_group?.toLowerCase() === "search" && c.config_name === "GOOGLE_API_KEY")?.config_value;

					const searchEngineId = configs.find((c: any) => c.config_group?.toLowerCase() === "search" && c.config_name === "SEARCH_ENGINE_ID")?.config_value;
			...
			fetchEventSource(api, {
				...
				headers: {
					...
					Authorization: type == "replay" ? `Bearer ${token}` : (undefined as unknown as string),
				},
				body: !type
					? JSON.stringify({
							project_id: project_id,
				...
				async onmessage(event: any) {
					...
					const isTaskSwitchingEvent = agentMessages.step === "confirmed" || agentMessages.step === "new_task_state" || agentMessages.step === "end";
					...
					if (agentMessages.step === "decompose_text") {
						...
						const currentContent = streamingDecomposeTextBuffer[currentId] || getCurrentChatStore().tasks[currentId]?.streamingDecomposeText || "";
								...
								if (agent_name == "browser_agent") {
									snapshots.forEach((item: any) => {
										const imgurl = !item.image_path.includes("/public")
											? item.image_path
											: (import.meta.env.DEV ? import.meta.env.VITE_PROXY_URL : import.meta.env.VITE_BASE_URL) + item.image_path;
						...
						if (agentMessages.step === "deactivate_agent") {
							if (message) {
								const index = taskAssigning[agentIndex].log.findLastIndex(
									(log) => log.data.method_name === agentMessages.data.method_name && log.data.toolkit_name === agentMessages.data.toolkit_name,
								);
								if (index != -1) {
					...
					if (agentMessages.step === "assign_task") {
						...
						const target = taskAssigning
							.map((agent, agentIndex) => {
								...
								const taskIndex = agent.tasks.findIndex((task: TaskInfo) => task.id === task_id && !task.reAssignTo);
						...
						if (taskState !== "waiting" && failure_count && failure_count > 0) {
							taskAssigning[assigneeAgentIndex].log = taskAssigning[assigneeAgentIndex].log.filter((log) => log.data.process_task_id !== task_id);
						}
					...
					if (agentMessages.step === "activate_toolkit") {
						// add log
						let taskAssigning = [...tasks[currentTaskId].taskAssigning];
						const resolvedProcessTaskId = resolveProcessTaskIdForToolkitEvent(tasks, currentTaskId, agentMessages.data.agent_name, agentMessages.data.process_task_id);
					...
					if (agentMessages.step === "deactivate_toolkit") {
							...
							if (message) {
								...
								if (task) {
									let index = task.toolkits?.findIndex((toolkit: any) => {
										return (
											toolkit.toolkitName === agentMessages.data.toolkit_name && toolkit.toolkitMethods === agentMessages.data.method_name && toolkit.toolkitStatus === "running"
										);
									});
						...
						const taskIndex = taskRunning.findIndex((task) => task.agent?.type === agentMessages.data.agent_name && task.toolkits?.at(-1)?.toolkitName === toolkit_name);
					...
					if (agentMessages.step === "error") {
						try {
							...
							const errorMessage =
								agentMessages.data?.message || (typeof agentMessages.data === "string" ? agentMessages.data : null) || "An error occurred while processing your request";
						...
						try {
							...
							if (project_id) {
								...
								if (project && project.queuedMessages) {
									const messageToRemove = project.queuedMessages.find((msg) => msg.task_id === taskIdToRemove || msg.content.includes(taskIdToRemove));
									if (messageToRemove) {
					...
					if (agentMessages.step === "end") {
						...
						Promise.all(tasks[currentTaskId].snapshotsTemp.map((snapshot) => proxyFetchPost(`/api/chat/snapshots`, { ...snapshot })));

						// Async file upload
						let res = await window.ipcRenderer.invoke("get-file-list", email, currentTaskId, (project_id || projectStore.activeProjectId) as string);
						if (!type && import.meta.env.VITE_USE_LOCAL_PROXY !== "true" && res.length > 0) {
							...
							const successCount = uploadResults.filter((result) => result.status === "fulfilled" && result.value.success).length;

							// Log failures
							const failures = uploadResults.filter((result) => result.status === "rejected" || (result.status === "fulfilled" && !result.value.success));
					...
					if (agentMessages.step === "ask") {
						if (tasks[currentTaskId].activeAsk != "") {
							const newMessage: Message = {
								...
								content:
									agentMessages.data?.content || agentMessages.data?.notice || agentMessages.data?.answer || agentMessages.data?.question || (agentMessages.data as string) || "",
								step: agentMessages.step,
								isConfirm: false,
							};
					...
					const newMessage: Message = {
						id: generateUniqueId(),
						role: "agent",
						content: agentMessages.data?.content || agentMessages.data?.notice || agentMessages.data?.answer || agentMessages.data?.question || (agentMessages.data as string) || "",
						step: agentMessages.step,
						isConfirm: false,
					};
					addMessages(currentTaskId, newMessage);
				...
				onerror(err) {
					...
					if (err instanceof TypeError || err?.message?.includes("Failed to fetch") || err?.message?.includes("ECONNREFUSED") || err?.message?.includes("NetworkError")) {
			...
			set((state) => {
				...
				return {
					...
					tasks: {
						...
						[taskId]: {
							...state.tasks[taskId],
							messages: state.tasks[taskId].messages.filter((message) => message.id !== messageId),
						},
					},
				};
			});
		...
		updateTaskInfo(index: number, content: string) {
			...
			const targetTaskInfo = tasks[activeTaskId].taskInfo.map((item, i) => (i === index ? { ...item, content } : item));
			setTaskInfo(activeTaskId, targetTaskInfo);
		},
