			...
			async onmessage(event: any) {
				...
				if (agentMessages.step === "to_sub_tasks") {
					...
					if (toSubTaskIndex === -1) {
						...
						setTimeout(() => {
							...
							const message = tasks[taskId].messages.findLast(
								(item) => item.step === "to_sub_tasks",
							);
							const isConfirm = message?.isConfirm || false;
							const isTakeControl = tasks[taskId].isTakeControl;
				...
				if (agentMessages.step === "create_agent") {
					...
					if (
						...
						const hasAgent = tasks[taskId].taskAssigning.find(
				...
				if (agentMessages.step === "task_state") {
					...
					const targetTaskAssigningIndex = taskAssigning.findIndex((agent) =>
						agent.tasks.find((task: TaskInfo) => task.id === task_id),
					);
					if (targetTaskAssigningIndex !== -1) {
						...
						// destroy webview
						tasks[taskId].taskAssigning = tasks[taskId].taskAssigning.map(
							(item) => {
								if (
									item.type === "search_agent" &&
				...
				if (agentMessages.step === "assign_task") {
					if (!agentMessages.data?.assignee_id || !agentMessages.data?.task_id)
						return;
					...
					if (taskAssigning && taskAssigning[assigneeAgentIndex]) {
						const exist = taskAssigning[assigneeAgentIndex].tasks.find(
							(item) => item.id === task_id,
						);
						if (exist) {
							...
							taskAssigning[assigneeAgentIndex].tasks.push(
								task ?? { id: task_id, content, status: "running" },
							);
						}
					}
				...
				if (agentMessages.step === "activate_toolkit") {
					...
					if (
						...
						addWebViewUrl(
							taskId,
							agentMessages.data.message as string,
							agentMessages.data.process_task_id as string,
						);
					}
					if (
					...
					if (
						...
						addWebViewUrl(
							taskId,
							JSON.parse(agentMessages.data.message)?.url as string,
							agentMessages.data.process_task_id as string,
						);
					}
					let taskRunning = [...tasks[taskId].taskRunning];
					...
					const taskIndex = taskRunning.findIndex(
						(task) => task.id === agentMessages.data.process_task_id,
					);
				...
				if (agentMessages.step === "end") {
					...
					Promise.all(
						tasks[taskId].snapshotsTemp.map((snapshot) =>
					...
					const fileList = tasks[taskId].taskAssigning
						.map((agent) => {
							return agent.tasks
								.map((task) => {
					...
					const agent_summary_end = tasks[taskId].messages.findLast(
