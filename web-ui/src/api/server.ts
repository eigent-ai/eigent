...
export async function fetchCurrentUser(): Promise<UserAccount> {
  return proxyFetchGet('/api/v1/user');
}

export async function updateUserProfile(
  profile: UserProfile
): Promise<UserProfile> {
  return proxyFetchPut('/api/v1/user/profile', profile);
}

export async function fetchBillingSummary(): Promise<BillingSummary> {
  return proxyFetchGet('/api/v1/user/billing-summary');
}
...
export async function fetchChatSteps(taskId: string) {
  return proxyFetchGet('/api/v1/chat/steps', { task_id: taskId });
}

export async function fetchChatSnapshots(apiTaskId: string) {
  return proxyFetchGet('/api/v1/chat/snapshots', { api_task_id: apiTaskId });
}

export async function autoLoginLocal(): Promise<void> {
  const response = await proxyFetchPost('/api/v1/user/auto-login');
