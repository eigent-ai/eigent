...
export function SessionList({
  ...
  if (project.sessions.length === 0) {
    return (
      <div className="rounded-lg border-ds-border-neutral-subtle-disabled p-4 text-body-sm text-ds-text-neutral-muted-default border border-dashed">
      ...
      {project.sessions.map((session) => (
        ...
        >
          <div className="mb-1 gap-2 flex items-center justify-between">
            <span
              className={cn(
                'px-2 py-0.5 text-body-sm rounded-full capitalize',
          ...
          <p className="text-body-md text-ds-text-neutral-default-default line-clamp-2">
            {session.question}
          </p>
          {session.createdAt ? (
