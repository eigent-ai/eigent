...
class RedisSessionManager:
    ...
    def store_session(self, session_id: str, user_id: str, metadata: dict[str, Any] | None = None) -> bool:
        ...
        try:
            ...
            logger.debug("Session stored in Redis", extra={"session_id": session_id, "user_id": user_id})
            return True

        except Exception as e:
            logger.error(
                "Failed to store session in Redis", extra={"session_id": session_id, "error": str(e)}, exc_info=True
            )
            return False
    ...
    def get_session(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error("Failed to get session from Redis", extra={"session_id": session_id, "error": str(e)})
            return None
    ...
    def remove_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.debug("Session removed from Redis", extra={"session_id": session_id, "user_id": user_id})
            return True

        except Exception as e:
            logger.error(
                "Failed to remove session from Redis", extra={"session_id": session_id, "error": str(e)}, exc_info=True
            )
            return False
    ...
    def get_user_sessions(self, user_id: str) -> set[str]:
        ...
        try:
            ...
            logger.error("Failed to get user sessions from Redis", extra={"user_id": user_id, "error": str(e)})
            return set()
    ...
    def add_pending_execution(self, session_id: str, execution_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to add pending execution",
                extra={"session_id": session_id, "execution_id": execution_id, "error": str(e)},
            )
            return False
    ...
    def remove_pending_execution(self, session_id: str, execution_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to remove pending execution",
                extra={"session_id": session_id, "execution_id": execution_id, "error": str(e)},
            )
            return False
    ...
    def get_pending_executions(self, session_id: str) -> set[str]:
        ...
        try:
            ...
            logger.error("Failed to get pending executions", extra={"session_id": session_id, "error": str(e)})
            return set()
    ...
    def update_session_ttl(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to update session TTL", extra={"session_id": session_id, "error": str(e)})
            return False
    ...
    def confirm_delivery(self, execution_id: str, session_id: str) -> bool:
        ...
        try:
            ...
            confirmation_data = json.dumps(
                {"execution_id": execution_id, "session_id": session_id, "delivered_at": datetime.now(UTC).isoformat()}
            )
            self.client.setex(confirmation_key, self.DELIVERY_TTL, confirmation_data)
            logger.debug("Delivery confirmed", extra={"execution_id": execution_id, "session_id": session_id})
            return True
        except Exception as e:
            logger.error(
                "Failed to confirm delivery",
                extra={"execution_id": execution_id, "session_id": session_id, "error": str(e)},
            )
            return False

    async def wait_for_delivery(
        self, execution_id: str, timeout: float = 10.0, poll_interval: float = 0.1
    ) -> dict[str, Any] | None:
        ...
        while elapsed < timeout:
            try:
                ...
                logger.error(
                    "Error checking delivery confirmation", extra={"execution_id": execution_id, "error": str(e)}
                )

            await asyncio.sleep(poll_interval)
            elapsed += poll_interval

        logger.warning("Delivery confirmation timeout", extra={"execution_id": execution_id, "timeout": timeout})
        return None
    ...
    def has_active_sessions_for_user(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to check user sessions", extra={"user_id": user_id, "error": str(e)})
            return False
    ...
    def publish_execution_event(self, event_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Published execution event to Redis",
                extra={"execution_id": event_data.get("execution_id"), "type": event_data.get("type")},
            )
            return True
        except Exception as e:
            logger.error("Failed to publish execution event", extra={"error": str(e)}, exc_info=True)
            return False
    ...
    async def subscribe_to_execution_events(self, callback: Callable[[dict[str, Any]], None]):
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(None, self._pubsub.subscribe, self.PUBSUB_CHANNEL)

            logger.info("Subscribed to execution events", extra={"channel": self.PUBSUB_CHANNEL})
            ...
            while True:
                ...
                if message and message["type"] == "message":
                    try:
                        ...
                        logger.error("Error processing pub/sub message", extra={"error": str(e)}, exc_info=True)

                # Small sleep to prevent tight loop
                await asyncio.sleep(0.01)

        except Exception as e:
            logger.error("Pub/sub subscription error", extra={"error": str(e)}, exc_info=True)


# Global instance
_redis_manager: RedisSessionManager | None = None
