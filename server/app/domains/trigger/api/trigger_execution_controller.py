...
async def create_trigger_execution(
    ...
    try:
        ...
        db_session.rollback()
        logger.error(
            "Trigger execution creation failed",
            extra={"user_id": auth.id, "trigger_id": data.trigger_id, "error": str(e)},
            exc_info=True,
        )
...
def get_execution(
    ...
    execution = db_session.exec(
        ...
        .join(Trigger)
        .where(and_(TriggerExecution.execution_id == execution_id, Trigger.user_id == str(auth.id)))
    ).first()
    if not execution:
...
async def update_execution(
    ...
    try:
        ...
        logger.error(
            ...
            extra={"user_id": auth.id, "execution_id": execution_id, "error": str(e)},
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def subscribe_executions(websocket: WebSocket):
    ...
    try:
        ...
        if not auth_token:
            await websocket.send_json(
                {"type": "error", "message": "Authentication required. Provide 'auth_token' in subscription message"}
            )
            await websocket.close()
            return
        ...
        try:
            ...
            user_id = auth.id
            logger.info(
                f"User authenticated for WebSocket {user_id} and {session_id}",
                extra={"user_id": user_id, "session_id": session_id},
            )
        except Exception as e:
            await websocket.send_json({"type": "error", "message": "Authentication failed"})
            await websocket.close()
            logger.warning("WebSocket authentication failed", extra={"session_id": session_id, "error": str(e)})
            return
        ...
        logger.info(
            ...
            extra={"session_id": session_id, "user_id": user_id, "total_active": len(active_websockets)},
        )

        await websocket.send_json(
            {"type": "connected", "session_id": session_id, "timestamp": datetime.now(UTC).isoformat()}
        )

        logger.info(
            "Client subscribed to executions",
            extra={
                ...
                try:
                    ...
                    if msg.get("type") == "ack" and msg.get("execution_id"):
                        ...
                        execution = ws_db_session.exec(
                            select(TriggerExecution).where(TriggerExecution.execution_id == execution_id)
                        ).first()
                        ...
                        if execution and execution.status == ExecutionStatus.pending:
                            ...
                            logger.info(
                                "Execution acknowledged and started",
                                extra={"session_id": session_id, "execution_id": execution_id},
                            )

                            await websocket.send_json(
                                {"type": "ack_confirmed", "execution_id": execution_id, "status": "running"}
                            )
        ...
        await asyncio.gather(handle_messages(), send_heartbeat(), return_exceptions=True)
...
async def handle_pubsub_message(event_data: dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"[PUBSUB] Received event from Redis: {event_type}",
            extra={
        ...
        if event_type == "pong":
            ...
            if target_session_id and target_session_id in active_websockets:
                try:
                    ...
                    await ws.send_json({"type": "pong", "timestamp": event_data.get("timestamp")})
                    logger.debug("Pong sent via Redis pub/sub", extra={"session_id": target_session_id})
                except Exception as e:
                    logger.error("Failed to send pong", extra={"session_id": target_session_id, "error": str(e)})
            return
        ...
        if not event_user_id:
            logger.warning("Event missing user_id, cannot filter subscribers", extra={"execution_id": execution_id})
            return
        ...
        logger.debug(
            f"User has {len(user_session_ids)} active session(s)",
            extra={"user_id": event_user_id, "session_count": len(user_session_ids)},
        )
        ...
        if not local_sessions:
            logger.debug(
                "No local WebSocket connections for this user",
                extra={"user_id": event_user_id, "execution_id": execution_id},
            )
            return  # No local connections for this user

        logger.info(
            f"Broadcasting execution to {len(local_sessions)} WebSocket(s)",
            extra={"execution_id": execution_id, "user_id": event_user_id, "session_count": len(local_sessions)},
        )
        ...
        for session_id in local_sessions:
            try:
                ...
                logger.debug(
                    "Notified session of execution",
                    extra={"session_id": session_id, "user_id": event_user_id, "execution_id": execution_id},
                )

            except Exception as e:
                logger.error("Failed to notify session", extra={"session_id": session_id, "error": str(e)})
                disconnected_sessions.append(session_id)
        ...
        if notified_count > 0:
            logger.debug(
                "Execution event broadcast complete",
                extra={"execution_id": execution_id, "user_id": event_user_id, "sessions_notified": notified_count},
            )

    except Exception as e:
        logger.error(
            f"Error handling pub/sub message: {str(e)}",
            extra={"error": str(e), "error_type": type(e).__name__, "event_data": event_data},
            exc_info=True,
        )
