...
async def webhook_trigger(webhook_uuid: str, request: Request, db_session: Session = Depends(session)):
    ...
    try:
        ...
        if not trigger:
            logger.warning("Webhook trigger not found or inactive", extra={"webhook_uuid": webhook_uuid})
        ...
        if handler:
            ...
            if trigger.status == TriggerStatus.pending_verification:
                ...
                logger.info(
                    "Trigger status updated to active after receiving valid event",
                    extra={"trigger_id": trigger.id, "trigger_type": trigger.trigger_type.value},
                )
        ...
        if trigger.trigger_type == TriggerType.webhook and trigger.webhook_method:
            expected_method = (
                trigger.webhook_method.value
                if hasattr(trigger.webhook_method, "value")
                else str(trigger.webhook_method)
            )
            expected_method = expected_method.rstrip(",")
            if request.method.upper() != expected_method.upper():
                raise HTTPException(
                    status_code=405, detail=f"Method not allowed. This webhook only accepts {expected_method} requests"
                )
        ...
        if handler:
            ...
            if trigger.trigger_type == TriggerType.webhook:
                filter_result = await handler.filter_event(input_data, trigger, headers=safe_headers, body_raw=body_raw)
            else:
                filter_result = await handler.filter_event(input_data, trigger)

            if not filter_result.success:
                logger.debug("Event filtered", extra={"trigger_id": trigger.id, "reason": filter_result.reason})
        ...
        if trigger.max_executions_per_hour or trigger.max_executions_per_day:
            if not check_rate_limits(db_session, trigger):
                logger.warning("Webhook rate limit exceeded", extra={"trigger_id": trigger.id})
        ...
        if trigger.is_single_execution:
            ...
            execution_count = db_session.exec(
                select(func.count(TriggerExecution.id)).where(TriggerExecution.trigger_id == trigger.id)
            ).first()
        ...
        try:
            ...
            if has_active_sessions:
                ...
                delivery_confirmation = await redis_manager.wait_for_delivery(execution_id, timeout=10.0)

                if delivery_confirmation:
                    logger.info(
                        "Webhook delivery confirmed",
                        extra={"execution_id": execution_id, "session_id": delivery_confirmation.get("session_id")},
                    )
                    return {
                        "success": True,
                        "execution_id": execution_id,
                    ...
                    logger.warning(
                        "Webhook delivery confirmation timed out",
                        extra={"execution_id": execution_id, "trigger_id": trigger.id},
                    )
                    return {
                        "success": True,
                        "execution_id": execution_id,
                ...
                logger.info(
                    "No active WebSocket sessions for user",
                    extra={"execution_id": execution_id, "user_id": trigger.user_id},
                )
                return {
                    "success": True,
                    "execution_id": execution_id,
        ...
        logger.error(
            "Webhook trigger processing failed", extra={"webhook_uuid": webhook_uuid, "error": str(e)}, exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/trigger/{webhook_uuid}/info", name="webhook info")
def get_webhook_info(webhook_uuid: str, db_session: Session = Depends(session)):
