...
from app.model.trigger.app_configs import ScheduleTriggerConfig, SlackTriggerConfig, WebhookTriggerConfig
from app.model.trigger.trigger import Trigger
from app.shared.types.config_group import ConfigGroup
from app.shared.types.trigger_types import ExecutionType, TriggerStatus, TriggerType
...
class BaseAppHandler:
    ...
    async def get_credentials(self, session: Session, user_id: str) -> dict:
        ...
        configs = session.exec(
            select(Config).where(and_(Config.user_id == int(user_id), Config.config_group == self.config_group))
        ).all()
        return {config.config_name: config.config_value for config in configs}

    async def authenticate(self, request: Request, body: bytes, trigger: Trigger, session: Session) -> AppHandlerResult:
        """
    ...
    async def filter_event(self, payload: dict, trigger: Trigger) -> AppHandlerResult:
        """
    ...
    def normalize_payload(self, payload: dict, trigger: Trigger, request_meta: dict = None) -> dict:
...
class SlackAppHandler(BaseAppHandler):
    ...
    async def authenticate(self, request: Request, body: bytes, trigger: Trigger, session: Session) -> AppHandlerResult:
        ...
        if not slack_auth.verify_webhook_request(request, body):
            logger.warning("Invalid Slack webhook signature", extra={"trigger_id": trigger.id})
...
class DefaultWebhookHandler(BaseAppHandler):
    ...
    async def filter_event(
        self, payload: dict, trigger: Trigger, headers: dict = None, body_raw: str = None
    ) -> AppHandlerResult:
        ...
        should_trigger, reason = config.should_trigger(body=body_raw or "", headers=headers or {}, text=text)
...
class ScheduleAppHandler(BaseAppHandler):
    ...
    def check_and_handle_expiration(self, trigger: Trigger, session: Session) -> bool:
        """
        ...
        try:
            ...
            logger.warning(
                "Invalid schedule config during expiration check", extra={"trigger_id": trigger.id, "error": str(e)}
            )
            return False
    ...
    def validate_schedule_for_execution(self, trigger: Trigger) -> tuple[bool, str]:
        """
...
def get_app_handler(trigger_type: TriggerType) -> BaseAppHandler | None:
