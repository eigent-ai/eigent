...
from app.core.trigger_utils import MAX_DISPATCH_PER_TICK, check_rate_limits
...
class TriggerScheduleService:
    ...
    def fetch_due_schedules(self, limit: int | None = 100) -> list[Trigger]:
        """
        ...
        try:
            ...
            logger.debug("Fetched due schedules", extra={"count": len(results), "current_time": now.isoformat()})

            return list(results)

        except Exception as e:
            logger.error("Failed to fetch due schedules", extra={"error": str(e)}, exc_info=True)
            return []

    def calculate_next_run_at(self, trigger: Trigger, base_time: datetime | None = None) -> datetime:
        """
        ...
        try:
            ...
            logger.error(
                ...
                extra={"trigger_id": trigger.id, "cron_expression": trigger.custom_cron_expression, "error": str(e)},
            )
            raise
    ...
    def dispatch_trigger(self, trigger: Trigger) -> bool:
        ...
        try:
            ...
            execution = TriggerExecution(
                ...
                input_data={"scheduled_at": datetime.now(UTC).isoformat()},
            ...
            try:
                trigger.next_run_at = self.calculate_next_run_at(trigger, datetime.now(UTC))
            except Exception as e:
                logger.error(
                ...
                trigger.next_run_at = datetime.now(UTC) + timedelta(days=365)

            # If single execution, deactivate the trigger
            if trigger.is_single_execution:
                trigger.status = TriggerStatus.inactive
                logger.info("Trigger deactivated after single execution", extra={"trigger_id": trigger.id})
            ...
            try:
                ...
                logger.debug(
                    "WebSocket notification sent", extra={"execution_id": execution_id, "trigger_id": trigger.id}
                )
            except Exception as e:
                # Don't fail the trigger dispatch if notification fails
                logger.warning(
                    "Failed to send WebSocket notification",
                    extra={"trigger_id": trigger.id, "execution_id": execution_id, "error": str(e)},
                )

            return True

        except Exception as e:
            logger.error("Failed to dispatch trigger", extra={"trigger_id": trigger.id, "error": str(e)}, exc_info=True)
    ...
    def poll_and_execute_due_triggers(self, max_dispatch_per_tick: int | None = None) -> tuple[int, int]:
        """
        ...
        while True:
            ...
            logger.debug("Batch processed", extra={"dispatched": dispatched_count, "rate_limited": rate_limited_count})
        ...
        if total_dispatched > 0 or total_rate_limited > 0:
            logger.info(
                ...
                extra={"total_dispatched": total_dispatched, "total_rate_limited": total_rate_limited},
            )
    ...
    def _check_schedule_valid(self, trigger: Trigger) -> bool:
        ...
        try:
            ...
            logger.warning("Invalid schedule config", extra={"trigger_id": trigger.id, "error": str(e)})
            return False
    ...
    def update_trigger_next_run(self, trigger: Trigger) -> None:
        ...
        try:
            ...
            if not self._check_schedule_valid(trigger):
                logger.info("Trigger expired, not updating next_run_at", extra={"trigger_id": trigger.id})
                return
            ...
            logger.info(
                ...
                extra={"trigger_id": trigger.id, "next_run_at": trigger.next_run_at.isoformat()},
            )
        except Exception as e:
            logger.error("Failed to update trigger next_run_at", extra={"trigger_id": trigger.id, "error": str(e)})
            self.session.rollback()
