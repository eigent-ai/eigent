...
class BaseTriggerConfig(BaseModel):
    ...
    max_failure_count: int | None = Field(
        default=None,
    ...
    def matches_filter(self, text: str | None) -> bool:
        """
    ...
    def get_required_config_group(self) -> ConfigGroup | None:
        """
    ...
    def check_activation_requirements(self, user_id: int, session: "Session") -> dict[str, Any]:
        """
        ...
        if not self.authentication_required:
            return {"can_activate": True, "missing_requirements": [], "message": "Authentication not required"}
        ...
        if not config_group or not required_credentials:
            return {"can_activate": True, "missing_requirements": [], "message": "No specific credentials required"}
        ...
        configs = session.exec(
            select(Config).where(and_(Config.user_id == int(user_id), Config.config_group == config_group.value))
        ).all()
        ...
        missing = [cred for cred in required_credentials if cred not in available_credentials]
        ...
        return {"can_activate": True, "missing_requirements": [], "message": "All requirements met"}
    ...
    def validate_activation(self, user_id: int, session: "Session") -> None:
        ...
        if not result["can_activate"]:
            raise ActivationError(message=result["message"], missing_requirements=result["missing_requirements"])
