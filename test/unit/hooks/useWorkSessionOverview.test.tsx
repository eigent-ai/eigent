...
import { buildWorkSessionOverview } from '@/hooks/useWorkSessionOverview';
import {
...
describe('buildWorkSessionOverview', () => {
  it('keeps an active durable Run current even when history is newer', () => {
    const overview = buildWorkSessionOverview(snapshot());
  ...
  it('keeps canonical Run sequence order when timestamps are reversed', () => {
    ...
    const overview = buildWorkSessionOverview(input);
  ...
  it('returns an empty view before durable hydration has a snapshot', () => {
    expect(buildWorkSessionOverview(null)).toEqual({
      currentRun: null,
      historicalRuns: [],
      runs: [],
    });
