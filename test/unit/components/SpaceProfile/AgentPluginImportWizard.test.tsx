...
import { AgentPluginImportWizard } from '@/components/SpaceProfile/AgentPluginImportWizard';
...
describe('AgentPluginImportWizard', () => {
  ...
  it('requires explicit review confirmation and binds conversion to the reviewed digest', async () => {
    ...
    expect(
      screen.getByText(/has not been published or imported elsewhere/i)
    ).toBeVisible();

    await user.click(
      screen.getByRole('button', { name: /review space settings/i })
    );
  ...
  it('refreshes target state and requires a new request after a definitive draft conflict', async () => {
    ...
    mocks.convert
      .mockRejectedValueOnce(
        Object.assign(new Error('Space settings changed.'), {
          status: 409,
          response: {
            data: {
    ...
    expect(await screen.findByText(/space settings changed/i)).toBeVisible();
    await user.click(convertButton);
  ...
  it('requires a second explicit confirmation before replacing an existing Space profile draft', async () => {
    const user = userEvent.setup();
    ...
    expect(
      await screen.findByText(/replace the existing space profile draft/i)
    ).toBeVisible();
    expect(screen.getByText(/does not merge settings/i)).toBeVisible();
    expect(mocks.convert).not.toHaveBeenCalled();

    await user.click(
      screen.getByRole('checkbox', {
        name: /confirm replacing existing space profile draft/i,
      })
    );
    await user.click(convertButton);
  ...
  it('locks the target selector while target preflight is pending', async () => {
    ...
    const target = screen.getByRole('combobox', { name: /target space/i });
