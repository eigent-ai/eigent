...
import { SpaceProfileImportWizard } from '@/components/SpaceProfile/SpaceProfileImportWizard';
...
function renderWizard(props: {
  ...
  return render(
    <MemoryRouter>
      <SpaceProfileImportWizard {...props} />
    </MemoryRouter>
  );
}

describe('SpaceProfileImportWizard', () => {
  beforeEach(() => {
  ...
  it('renders without standalone page navigation', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Import Space profile' })
    ).toBeInTheDocument();
  });
  ...
  it('keeps local setup editable after the Space profile is imported', async () => {
    ...
    expect(await screen.findByText('Space files imported')).toBeInTheDocument();
