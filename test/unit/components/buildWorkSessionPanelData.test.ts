...
describe('buildWorkSessionPanelData', () => {
  it('deduplicates logical agents across Runs and skips anonymous tool frames', () => {
    ...
    const data = buildWorkSessionPanelData([oldRun, currentRun], []);
  ...
  it('classifies remote delegated agents separately from primary agents', () => {
    ...
    expect(buildWorkSessionPanelData([run], []).agents).toMatchObject([
  ...
  it('projects plans, artifacts and safe URL resources across Runs', () => {
    ...
    const data = buildWorkSessionPanelData([current, historical], []);
  ...
  it('does not enrich by basename or an unsafe relative path', () => {
    const item = buildWorkSessionPanelData(
      [
  ...
  it('shows a trusted realtime write and converges on its terminal artifact', () => {
    ...
    const liveData = buildWorkSessionPanelData(
    ...
    const finalized = buildWorkSessionPanelData(
  ...
  it('accepts a real todo lifecycle without authorizing a later stale state', () => {
    ...
    expect(buildWorkSessionPanelData([run], []).progress).toMatchObject([
  ...
  it('shows only explicitly loaded skills and ignores skill discovery', () => {
    ...
    expect(buildWorkSessionPanelData([run], []).contextItems).toMatchObject([
      {
        id: 'skill:pdf',
        label: 'pdf',
        category: 'skill',
