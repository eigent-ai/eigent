...
import { SpaceSettingsEditor } from '@/pages/SpaceSettings';
import {
  fireEvent,
  render,
  screen,
...
vi.mock('@/components/SpaceSettings/ShareSpaceProfileDialog', () => ({
  ShareSpaceProfileDialog: () => null,
}));
...
describe('SpaceSettingsEditor', () => {
  ...
  it('registers pending changes with the shared navigation guard', async () => {
    ...
    const { unmount } = render(
      <SpaceSettingsEditor presentation="settings" spaceId="space-1" />
    );
  ...
  it('keeps only tabs in the settings rail and groups version status with Space profile sharing', async () => {
    const { container } = render(
      <SpaceSettingsEditor presentation="settings" spaceId="space-1" />
    );
    ...
    expect(profileRow).toHaveTextContent('Space profile');
    ...
    expect(
      within(identityGroup).getByText(
        'Shown to collaborators and people who import this profile.'
      )
    ).toBeInTheDocument();
    ...
    const collectionSections = {
      ...
      'space-settings-context': 'Reference sources actions',
  ...
  it('keeps panel exit mounted and exposes only the current status announcement', async () => {
    ...
    rerender(<SpaceSettingsEditor presentation="settings" spaceId="space-1" />);
  ...
  it('tracks the current contents item while the settings sections scroll', async () => {
    ...
    await waitFor(() =>
      expect(
        within(contents).getByRole('button', { name: 'Reference sources' })
      ).toHaveAttribute('aria-current', 'location')
    );
  });
  ...
  it('uses picker-first creation for external resources and commits new items from the panel', async () => {
    ...
    const pickerFlows = [
      {
        ...
        addLabel: 'Add source',
        panelLabel: 'Add reference source',
        picker: 'context',
      },
      {
    ...
    expect(
      within(panel).getByRole('combobox', { name: 'Model preset' })
