...
export function createElectronAPIMock(): MockedElectronAPI {
  ...
  const installLogListeners: Array<
  ...
  const electronAPI: MockedElectronAPI = {
    ...
    checkAndInstallDepsOnUpdate: vi.fn().mockImplementation(async () => {
      ...
      const versionChanged =
        !versionFileExists || savedVersion !== currentVersion;
      const needsInstallation =
        allowForceInstall || versionChanged || !venvExists || !toolInstalled;
      ...
      if (needsInstallation) {
        ...
        if (!toolInstalled) {
          electronAPI.simulateInstallationLog(
            'stdout',
            'Command tools missing, starting installation...'
          );
        } else if (!venvExists) {
          electronAPI.simulateInstallationLog(
            'stdout',
            'Virtual environment missing, starting installation...'
          );
        } else if (versionChanged) {
          electronAPI.simulateInstallationLog(
            'stdout',
            'Version changed, starting installation...'
          );
        }
        ...
        return {
          success: true,
          message:
            'Version not changed, venv exists, and tools installed - skipped installation',
        };
      }
    }),
    ...
    onInstallDependenciesComplete: vi
      .fn()
      .mockImplementation(
        (
          callback: (data: {
    ...
    updateEnvBlock: vi
      .fn()
      .mockImplementation((lines: string[], kv: Record<string, string>) => {
...
export function createIpcRendererMock(): MockedIpcRenderer {
  return {
    invoke: vi
      .fn()
      .mockImplementation(async (channel: string, ..._args: any[]) => {
