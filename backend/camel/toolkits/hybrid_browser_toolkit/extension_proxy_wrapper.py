...
class ExtensionProxyWrapper:
    ...
    async def wait_for_stop_signal(self) -> Optional[Dict[str, Any]]:
        """Wait for a STOP_TASK signal from the extension UI."""
        return await self._stop_queue.get()
    ...
    async def _handle_message(self, data: Dict[str, Any]):
        ...
        if msg_type == "CDP_RESULT":
            ...
            # Signal to stop the current running task (separate queue
            # so it's not blocked behind the running step)
            await self._stop_queue.put({"type": "STOP_TASK"})
            self._log("info", "Stop task requested")
    ...
    async def open_browser(self, url: Optional[str] = None) -> Dict[str, Any]:
        ...
        target_tab = self._default_tab_id
        if target_tab is None:
            attach_result = await self.request_attach()
            if not attach_result.get("success"):
                raise RuntimeError(
                    "Failed to attach to browser tab: "
                    f"{attach_result.get('error', 'Unknown error')}"
                )
            target_tab = attach_result.get("tabId")
            if target_tab is None:
                raise RuntimeError("No tab ID returned from attach request")
            self._default_tab_id = target_tab

        # Navigate the attached tab if needed.
        if url and target_tab is not None:
            try:
                await self._send_command(
                    "Page.navigate", {"url": url}, tab_id=target_tab
                )
                await asyncio.sleep(1)
            except Exception as e:
                self._log("warning", f"Navigation to {url} failed: {e}")

        # Get current page info from the attached tab.
        snapshot = await self.get_page_snapshot(tab_id=target_tab)
    ...
    async def visit_page(
        ...
        try:
            ...
            if target_tab is not None and target_tab in self._tabs:
                self._tabs[target_tab]["aria_initialized"] = False
                self._tabs[target_tab]["url"] = url
            else:
                self._aria_module_initialized = False
            snapshot = await self.get_page_snapshot(tab_id=target_tab)
