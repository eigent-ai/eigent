...
def test_memory_mutations_enforce_capacity_cas_tombstone_and_idempotency(
    ...
    purged = journal.apply_memory_mutation(
        ...
        memory_id="memory-1",
        actor_type="user",
        reason="Permanently delete archived Memory",
        expected_version=2,
        now=3,
    )
    assert purged.entry is None
    assert (
        journal.list_memory_entries(
            "project", "project-1", include_deleted=True
        )
...
def test_pin_memory_mutation_toggles_list_priority(journal):
    journal.apply_memory_mutation(
        mutation_id="mutation-add",
        idempotency_key="request-add",
        operation="add",
        ...
        content="Keep this entry available.",
        kind="fact",
        token_count=4,
        created_by="user",
        source_trust="user_confirmed",
    ...
    pinned = journal.apply_memory_mutation(
        mutation_id="mutation-pin",
        idempotency_key="request-pin",
        operation="pin",
        scope_type="project",
        scope_id="project-1",
        memory_id="memory-1",
    ...
    assert pinned.entry is not None
    assert pinned.entry.pinned_by_user is True

    unpinned = journal.apply_memory_mutation(
        mutation_id="mutation-unpin",
        idempotency_key="request-unpin",
        operation="pin",
        scope_type="project",
        scope_id="project-1",
        memory_id="memory-1",
    ...
    assert unpinned.entry is not None
    assert unpinned.entry.pinned_by_user is False
