...
async def _record_completed_single_agent_display_fixture(path: Path):
    ...
    with SQLiteRunJournal(path) as journal:
        ...
        with patch(
            ...
            checkpoint("tool-1", first_step, "prepared")
            checkpoint("tool-1", first_step, "dispatched")
            await record(
                ActionTerminalData(
            ...
            # Delayed completion keeps the original authored Step, even after
            # the next todo has become current. The Run ID is not a todo ID.
            checkpoint("tool-1", first_step, "completed")
...
async def test_single_agent_display_fixture_matches_actual_writers(tmp_path):
    ...
    for tool, todo in zip(tools, ["todo-1", "todo-2"]):
        ...
        assert (
            payload["semantic"]["correlation"]["step_id"] == payload["step_id"]
        )
