...
class ProcessRecord:
    ...
    agent_name: str
    label: str
    observe: Callable[[], tuple[bool, int | None, bool]]
...
class TerminalProcessRegistry:
    ...
    def register(
        ...
        *,
        ...
        completed = [r for r in candidates if not r.observe()[0]]
        with self._lock:
    ...
    def _snapshot(self, record, known_version=None):
        running, exit_code, stopped = record.observe()
        status = (
            "stopping"
        ...
        with self._lock:
            ...
            result.update(status=status, exit_code=exit_code, can_stop=running)
