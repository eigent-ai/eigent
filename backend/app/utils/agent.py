...
def agent_model(
    ...
    # Separate parameters that should be passed as kwargs vs model_config_dict
    # Parameters like api_version for Azure should be passed as direct kwargs
    model_kwargs: dict[str, Any] = {}
    excluded_keys = ["model_platform", "model_type", "api_key", "url"]

    # Azure-specific parameters that should be passed as kwargs to ModelFactory.create()
    # These are used for model initialization, not for API calls
    azure_kwargs = ["api_version", "azure_deployment_name"]

    for k, v in extra_params.items():
        if k not in excluded_keys:
            if k in azure_kwargs:
                model_kwargs[k] = v
            else:
                model_config[k] = v
...
def search_agent(options: Chat):
    working_directory = get_working_directory(options)
    traceroot_logger.info(
        f"Creating search agent for project: {options.project_id} in directory: {working_directory}"
    )
    message_integration = ToolkitMessageIntegration(
        message_handler=HumanToolkit(
            options.project_id, Agents.search_agent
        ).send_message_to_user
    )
    ...
    terminal_toolkit = TerminalToolkit(
        options.project_id, Agents.search_agent, safe_mode=True, clone_current_env=False
    )
    ...
    note_toolkit = NoteTakingToolkit(
        options.project_id, Agents.search_agent, working_directory=working_directory
    )
    note_toolkit = message_integration.register_toolkits(note_toolkit)
