...
class HybridBrowserToolkit(BaseHybridBrowserToolkit, AbstractToolkit):
    ...
    def __init__(
        ...
        if user_data_dir is None:
            base_dir = "/Users/puzhen/Desktop/other_project/camel_pr/camel/User_Data"
            # Create base directory if it doesn't exist
            os.makedirs(base_dir, exist_ok=True)
            # Use session_id or generate a unique directory name
            if session_id:
                user_data_dir = os.path.join(base_dir, f"session_{session_id}")
            else:
                import uuid
                user_data_dir = os.path.join(base_dir, f"session_{str(uuid.uuid4())[:8]}")
        super().__init__(
            headless=headless,
    ...
    def clone_for_new_session(self,
        ...
        # Create a unique user_data_dir for the cloned session
        new_user_data_dir = None
        if self._user_data_dir:
            base_dir = "/Users/puzhen/Desktop/other_project/camel_pr/camel/User_Data"
            new_user_data_dir = os.path.join(base_dir, f"session_{new_session_id}")
            
            # Copy the existing user_data_dir to the new location if it exists
            if os.path.exists(self._user_data_dir) and not os.path.exists(new_user_data_dir):
                try:
                    shutil.copytree(self._user_data_dir, new_user_data_dir)
                    logger.info(f"Copied user_data_dir from {self._user_data_dir} to {new_user_data_dir}")
                except Exception as e:
                    logger.warning(f"Failed to copy user_data_dir: {e}")
                    # Create the directory even if copy fails
